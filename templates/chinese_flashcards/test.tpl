	...
	{foreach item=cardset key=sub from=$lesson->words}
		...
		<div data-role="page" id="card_{$card->word_index}.{$card->sub_lesson}">
			...
			<div data-role="content" class="card_content">
				...
				<div id="view_pin-eng-chi_card_{$card->id}">
			...
			<div data-role="footer" class="ui-grid-a">
				...
				{if $next_index < $lesson->word_idents|@count}
					<a href="{$app_root}book/{$lesson->book->short_name}/lesson/{$lesson->name}#card_{$next}" data-role="button" class="ui-btn-right nextCard" data-icon="arrow-r" data-inline="true" data-iconpos="notext">next</a>
				{/if}
				</div>
			</div><!-- /footer -->
		</div><!-- /page -->
